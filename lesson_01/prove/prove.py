...
def draw_square(tur, x, y, side, color='black', lock=None):
    """Draw Square"""
    if lock:
        lock.acquire()
    try:
...
def draw_circle(tur, x, y, radius, color='red', lock=None):
    if lock:
        lock.acquire()
    try:
        steps = 10
...
def draw_rectangle(tur, x, y, width, height, color='blue', lock=None):
    """Draw a rectangle"""
    if lock:
        lock.acquire()
    try:
...
def draw_triangle(tur, x, y, side, color='green', lock=None):
    """Draw a triangle"""
    if lock:
        lock.acquire()
    try:
...
def draw_squares(tur, lock=None):
    ...
    for x in range(-300, 350, 200):
        for y in range(-300, 350, 200):
            threading.Thread(target=draw_square, args=(tur, x - 50, y + 50, 100), kwargs={'lock': lock}).start()
...
def draw_circles(tur, lock=None):
    ...
    for x in range(-300, 350, 200):
        for y in range(-300, 350, 200):
            threading.Thread(target=draw_circle, args=(tur, x, y-2, 50), kwargs={'lock': lock}).start()
...
def draw_triangles(tur, lock=None):
    ...
    for x in range(-300, 350, 200):
        for y in range(-300, 350, 200):
            threading.Thread(target=draw_triangle, args=(tur, x-30, y-30+10, 60), kwargs={'lock': lock}).start()
...
def draw_rectangles(tur, lock=None):
    ...
    for x in range(-300, 350, 200):
        for y in range(-300, 350, 200):
            threading.Thread(target=draw_rectangle, args=(tur, x-10, y+5, 20, 15), kwargs={'lock': lock}).start()
...
def run_with_threads(tur, log, main_turtle):
    ...
    # Create threads for each group of shapes
    threads = []
    lock = threading.Lock()
